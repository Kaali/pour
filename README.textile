...
combinations where the key is the name of a field and the value is a vector of
vectors, where the inner vectors defines a validator and an error message if
that validation fails.  The validators must return either the input parameter
or a conversion of it, which is then passed on to the next validator; *nil*
designates an error.
...
<pre>
{:errors {:login "Login should be a valid email address"}, :values {:login nil, :password "12345"}}
</pre>
